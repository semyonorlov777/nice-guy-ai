-- =============================================================
-- Fix love-languages mode system_prompts: append the quick-reply format block.
--
-- Root cause (see docs/runbooks/chat-message-formatting.md):
--   The 6 tool modes (ll_self_analysis, ll_partner_analysis,
--   ll_relationship_map, ll_theory, ll_love_translator, ll_roleplay)
--   have no «ФОРМАТ QUICK REPLIES» block — so the model produces
--   plain-text lists, «Что дальше?» headings, dash bullets instead of
--   «ёлочки» that ChatWindow's parseQuickReplies can turn into buttons.
--
-- Each mode's system_prompt is 4.3–7.1 KB; we APPEND the same block
-- we added at the program level (with «не вопросы к AI» — these are
-- tool AIs, not an author).
-- =============================================================
