-- =============================================================
-- Fix razgovorny-gipnoz mode system_prompts: append the quick-reply format block.
--
-- Root cause (see docs/runbooks/chat-message-formatting.md):
--   The 5 tool modes (hypno_rapport, hypno_negotiate, hypno_detect,
--   hypno_suggest, hypno_trance) have no «ФОРМАТ QUICK REPLIES» block —
--   so the model produces plain-text lists, «Что дальше?» headings,
--   dash bullets instead of «ёлочки» that ChatWindow's parseQuickReplies
--   can turn into buttons.
--
-- APPEND the same block we added at the program level (with
-- «не вопросы к AI» — these are tool AIs, not an author).
-- =============================================================
...
WHERE pm.mode_template_id = mt.id
  ...
  AND p.slug = 'razgovorny-gipnoz'
  AND mt.key IN ('hypno_rapport', 'hypno_negotiate', 'hypno_detect',
                 'hypno_suggest', 'hypno_trance');
