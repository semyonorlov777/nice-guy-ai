-- =============================================================
-- Fix nice-guy mode system_prompts: append the quick-reply format block.
--
-- Root cause (see docs/runbooks/chat-message-formatting.md):
--   The programs for the 7 tool modes (ng_my_syndrome, ng_relationships,
--   ng_parents, ng_boundaries, ng_quiz, ng_theory, exercises) have no
--   «ФОРМАТ QUICK REPLIES» block — so the model produces plain-text
--   lists, «Что дальше?» headings, dash bullets instead of «ёлочки»
--   that ChatWindow's parseQuickReplies can turn into buttons.
--
-- Unlike 100-notes (which had a weak single-liner to replace), nice-guy
-- prompts contain nothing about quick replies, so we APPEND the same
-- block we already added at the program level.
-- =============================================================
