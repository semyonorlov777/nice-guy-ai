...
### 8. 🔴 Деплой + chip аудита (обязательно)

После зелёного линтера и UI-проверки — деплой и запуск аудита. Полная инструкция: `.claude/skills/book-to-modes/SKILL.md` Этап 6.

**Порядок строго:**

1. **§6.3 — chip-карточка через `mcp__ccd_session__spawn_task` (КРИТИЧЕСКАЯ ТОЧКА)** — ДО `git commit`. Title: «Аудит книги <slug> после деплоя». Аудит сам ждёт деплоя в начале своей работы.
2. **§6.4 — коммит + merge в main.**
3. **§6.5 — дождаться Vercel-деплоя (WebFetch до 5 минут).**
4. **§6.6 — финальный отчёт заказчику ровно из трёх строк** (только после §6.3 и §6.5).

**❌ ЗАПРЕЩЕНО:** делать коммит/merge до создания chip; писать финальный отчёт без chip; откладывать spawn_task «на потом, после деплоя».

**Страховка.** Проектный Stop-хук `.claude/hooks/check-book-spawn-task.ps1` блокирует завершение сессии, если в свежих коммитах есть `feat(<slug>):` + новый `scripts/seed-<slug>.sql`, а `spawn_task` с title «Аудит книги» не вызван. Если хук сработал — создай chip и попробуй снова.
...
| Калибровка пройдена — но следующий режим начинает «как первый раз» | Новый `chat_type` калибровки не добавлен в `CALIBRATION_CHAT_TYPES` в `lib/chat/prepare-context.ts` (хардкод whitelist). `appendCalibrationContext()` фильтрует только перечисленные chat_type | Добавь новый chat_type (напр. `'belousov_calibration'`) в массив. Линтер: `calibration-not-wired`. Прецедент: borba-za-vnimanie 2026-05 |
| Фото автора на лендинге — та же картинка, что и обложка книги | Автор изображён на обложке (`/books/<slug>/cover.jpg`), кто-то скопировал тот же файл в `/authors/<slug>.jpg` | Положи отдельное живое фото автора, не из обложки. Если в открытом доступе только обложка — запроси у заказчика. Линтер: `author-photo-equals-cover` (md5-сравнение). Прецедент: borba-za-vnimanie 2026-05 |
| AI в промпте натыкается на литерал `{{cross_mode_data}}` | Placeholder скопирован из шаблонов других книг (eq-2-0, mind-power, redecision, heroes-and-outlaws), но в коде НЕТ обработчика — Gemini видит фигурные скобки буквально | Удали строку из `programs.system_prompt` и `author_chat_system_prompt`. Runtime подмешивает кросс-режимные данные через `appendCalibrationContext` / `appendPortraitContext` / `appendTestScores`, placeholder не нужен. Линтер: `cross-mode-data-placeholder` |
