...
**Дата:** 2026-04-24
**Статус:** ✅ ЗАКРЫТО 2026-05-21 — реализовано в рамках общего рефакторинга скролла.
**Приоритет:** ~~nice-to-have~~ сделано

## Как было сделано

Перешли на «контейнерный скролл по модели ChatGPT/Claude», а не на настоящий body-scroll
(как изначально планировалось здесь). Свежие исследования 2026 показали, что ChatGPT
и Claude используют именно контейнерный скролл на `<main overflow-auto>` с `100dvh`,
а не page-scroll. Это снимает половину рисков из тикета: виртуализация работает, FAB
работает, isUserScrolledUp работает.

Подключили `use-stick-to-bottom` (Sam Denty / StackBlitz Labs), добавили spacer-паттерн
на последнем сообщении, починили клавиатуру на iOS (`useKeyboardInset`) и свайпы в
Telegram (`useTelegramMiniApp`). Полный список изменений — в CHANGELOG.md.

См. план: `~/.claude/plans/stateful-painting-peach.md` (исследование, риски, реализация).
