...
export const ISSP_SCALE_NAMES: Record<string, string> = {
  approval: "Зависимость от одобрения",
  contracts: "Скрытые контракты",
  suppression: "Подавление потребностей",
  control: "Контроль и стратегии",
  boundaries: "Границы и ассертивность",
  masculinity: "Мужская идентичность",
  attachment: "Отношения и привязанность",
};

export const ISSP_BLOCK_INSIGHTS: string[] = [
  "Зависимость от одобрения — это фундамент синдрома. Гловер пишет: когда мальчик не получает безусловного принятия, он учится «заслуживать» любовь. Каждый комплимент становится кислородом, а каждое неодобрение — угрозой.",
  "Скрытые контракты — невысказанные ожидания, которые обречены на провал. Ты даёшь, надеясь получить — но другой человек не знает правил игры. Разочарование неизбежно, потому что контракт подписан в одностороннем порядке.",
  "Подавление потребностей — «хорошие парни» убеждены, что иметь потребности — эгоизм. Они научились отодвигать себя на второй план и гордиться этим. Но подавленные потребности не исчезают — они копятся и взрываются.",
  "Контроль — иллюзия, что если всё делать правильно, мир будет безопасным. Гловер описывает это как «магическое мышление»: если я буду достаточно хорошим, ничего плохого не произойдёт.",
  "Границы — способность говорить «нет» без вины. Для «славного парня» отказ = конфликт, а конфликт = потеря любви. Поэтому он соглашается, накапливает обиду и в итоге взрывается или уходит молча.",
  "Мужская идентичность — многие «хорошие парни» выросли без здоровой мужской модели. Они отвергают в себе «мужские» качества — напористость, агрессию, сексуальность — считая их опасными или неприемлемыми.",
