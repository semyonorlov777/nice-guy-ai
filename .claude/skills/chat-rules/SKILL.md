...
11. **AI начинает ответ со «Здравствуй» / «Отличный вопрос»** — причина: нет блока «Запрет приветствий» (кирпич Б REFERENCE.md §5.0) в system_prompt. Без него Gemini здоровается в каждом втором ответе. Фикс: вставить блок во все 4 типа промптов программы.
12. **AI сваливается на «вы» (1 ответ из 3-4)** — причина: нет блока «ОБРАЩЕНИЕ — КРИТИЧЕСКОЕ ПРАВИЛО» (кирпич А) с явными контр-примерами. Фикс: вставить блок А с явными списками ПРАВИЛЬНО / НЕПРАВИЛЬНО.
13. **Демо-чат отвечает общими словами без названия концепта** — причина: в `anonymous_system_prompt` нет блока «СТРУКТУРА ПЕРВОГО ОТВЕТА» с `ОБЯЗАТЕЛЬНО назови ... по имени` (блок Д). Фикс: вставить блок + заполнить `landing_data.main_concepts`.
14. **Демо-чат выводит варианты простым текстом без кавычек** — причина: в `anonymous_system_prompt` нет блока «Quick replies — КРИТИЧЕСКОЕ ПРАВИЛО» (блок Е) с контр-примером «без кавычек — это НЕ кнопки». Фикс: вставить блок Е.
15. **Дубликат заголовка на карточке режима** — `welcome_title` повторяет `mode_templates.name` (тройной заголовок: меню + шапка + карточка). Фикс: `welcome_title` — фраза действия («Что ты знаешь о партнёре»), `mode_templates.name` — короткий ярлык («Карта любви»).
16. **HTML-теги (`<em>`, `<strong>`, `<br>`) видны буквально на лендинге** — причина: вставлены в поля без поддержки разметки (`landing_data.chat_header.title`, `book.*`, `personas.items[].body`). См. таблицу полей и разметки в runbook.
17. **Перемешанный порядок вопросов теста** — в `test_configs.questions[]` шкалы чередуются, между блоками показывается ложный заголовок «Следующий: Нежность» когда следующий вопрос не из нежности. Фикс: группировать вопросы по шкалам блоками `questions_per_block`.
