...
- [ ] **D1:** Промпты упоминают использование данных из других режимов (через явные ссылки на режим-источник, не через placeholder `{{cross_mode_data}}` — он не обрабатывается в коде; см. D8)
...
- [ ] **D7 (v1.7):** Если у программы есть режим калибровки (chat_type содержит `calibration`) — этот chat_type должен быть в массиве `CALIBRATION_CHAT_TYPES` в [lib/chat/prepare-context.ts](../../../../lib/chat/prepare-context.ts). Иначе калибровка декларативная: пройденные ответы пользователя не подмешиваются как контекст в следующий режим. Линтер `npm run check:chats` правило `calibration-not-wired`. Прецедент: borba-za-vnimanie 2026-05
- [ ] **D8 (v1.7):** Ни в одном `system_prompt` (`programs.*` или `program_modes.*`) нет литерала `{{cross_mode_data}}`. Placeholder скопирован из шаблонов 5+ книг, в коде НЕТ обработчика — Gemini видит фигурные скобки буквально. Runtime подмешивает кросс-режимные данные через `appendCalibrationContext` / `appendPortraitContext` / `appendTestScores`. Линтер: `cross-mode-data-placeholder`. Прецедент: borba-za-vnimanie 2026-05
...
- [ ] **H7 (v1.7):** `landing_data.author.photo_url` ≠ `landing_data.book.cover_url` (по содержимому файла, не пути). На лендинге секции «О книге» и «Об авторе» должны иметь разные изображения, даже если автор изображён на обложке. Линтер `npm run check:author-photos` правило `author-photo-equals-cover` (md5-сравнение). Прецедент: borba-za-vnimanie 2026-05
