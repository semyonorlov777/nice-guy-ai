...
#### Несколько авторов и фото — три кейса

Триггер: в исходных данных книги видишь «&», «and», «with», запятую между именами (`Daniel Priestley & Glen Carlson`, `Margaret Mark, Carol Pearson`, `Frank, Davidson`).

| Сколько авторов | Кто на фото | `landing_data.author.name` | `landing_data.author.credentials` | `landing_data.book.author_top` |
|---|---|---|---|---|
| Один | Один | «Имя Фамилия» | Биография про одного | «Имя Фамилия» |
| Несколько / на фото один | Главный (тот кто на фото) | «Имя главного» (без партнёра) | Биография про главного + одна фраза «Вместе с партнёром N соосновал…» | «N1 with N2» или «N1 и N2» (библиографически как на обложке) |
| Несколько / на фото все | Все | «N1 и N2» | Биография «вместе» | «N1 и N2» |

**Антипаттерн:** на фото один человек, но `author.name = "Имя1 & Имя2"`. Пользователь видит несоответствие — кто из них кто? Карточка теряет доверие.

**Прецеденты:**
- `scorecard-marketing` Пристли — сначала было `"Daniel Priestley & Glen Carlson"` при фото только Пристли. Постфактум переименовано в `"Дэниел Пристли"`, Глен упомянут в credentials как партнёр.
- `eq-2-0` Бредберри — фото только Бредберри, в `name` он один, на обложке (`book.author_top`) оба автора с Гривз. Сделано правильно изначально.
- `heroes-and-outlaws` Марк/Пирсон — в `name` только Пирсон (Марк умерла в 2000), на обложке обе.

#### Правило русского интерфейса

Все user-facing тексты программы — **на русском**. Английский разрешён только в библиографической записи книги.

**Обязательно русский (если попадёшь сюда с английским — рецидив):**
- `landing_data.author.name`, `author.credentials`, `author.quote`
- `landing_data.problem.*`, `outcomes.*`, `comparison.rows[].values`, `personas.items[].*`, `outcomes.items[].*`
- `landing_data.hub_messages.*`, `chat_header.*`
- Все `welcome_*` (`welcome_mode_label`, `welcome_title`, `welcome_subtitle`, `welcome_ai_message`, `welcome_replies`)
- Все `system_prompt` (`programs.system_prompt`, `author_chat_system_prompt`, `anonymous_system_prompt`, `program_modes.system_prompt`) — текст промпта на русском, даже если книга оригинально на английском
- `mode_templates.name` через `program_modes.welcome_mode_label`

**Английский разрешён (allowlist — это «обложка», не интерфейс):**
- `landing_data.book.title` — название книги на языке оригинала (`Scorecard Marketing`, `The Choice`)
- `landing_data.book.subtitle` — подзаголовок на языке оригинала
- `landing_data.book.author_top` — авторы как на обложке оригинала (`Daniel Priestley with Glen Carlson`)
- `landing_data.book.author_bottom` — credentials автора на обложке (если есть)
- `landing_data.main_concepts[]` — термины автора (`Psychological Tension`, `Dormant Desires`) допустимы, **но** в `anonymous_system_prompt` каждый термин должен встретиться с русской расшифровкой («Psychological Tension — психологическое напряжение, когда…»)

**Запрещено смешение** внутри одного русского поля: «Daniel — автор бестселлера…», «Запустили первый бизнес в 2002» (про двух — а в `name` один). Используй транслитерацию имён по справочнику [`AUTHOR_NAMES.md`](AUTHOR_NAMES.md).

**Исключение** — профильная клиническая книга `transdiagnostic-cbt`: `author.name = "Jacqueline B. Persons, PhD"` оставлен на английском намеренно, потому что ЦА — practicing psychologists, оригинальное написание узнаваемо. Это редкий случай, не образец. Применять только если у книги нет русского издания и ЦА — профессионалы.
...
**Минимум 5 строк в `rows`** (меньше — секция выглядит пустой). Типовые параметры: «Разбор твоего случая», «Скорость», «Голос автора», «Стоимость», + один уникальный для книги (например «Ролевые звонки» у Пристли, «Тест ISSP» у Гловера).

**Обязательная строка «Язык» — если книга или программа автора идут на иностранном языке.** Без этой строки пользователь не видит языковой барьер доступа — это важный фактор выбора между «купить книгу автора» / «пойти к нему на программу» / «взять наш сервис».

```json
{ "param": "Язык", "values": ["На английском", "На английском", "На русском"], "dim": [0, 1] }
```

**Правило:**
- Если книга в оригинале на русском (Гловер переведён → есть русское издание; Ильяхов, Белоусов — русские авторы) → строка «Язык» НЕ нужна.
- Если книга в оригинале на английском И есть русский перевод (Готтман, Чепмен, Берн, Бредберри) → строка «Язык» НЕ нужна — пользователь читает на русском.
- Если книги нет на русском И/ИЛИ программа автора идёт на английском (Пристли, Frank/Davidson) → строка «Язык» **обязательна** с dim=[0, 1].

**Прецедент:** `scorecard-marketing` Пристли — первая редакция лендинга вышла без строки «Язык». Постфактум добавили строку: «На английском / На английском / На русском».
