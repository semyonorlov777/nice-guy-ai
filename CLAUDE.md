...
AI-платформа тренажёров по книгам. 13 программ в БД, две группы:
- **Психология / отношения** (10 книг): nice-guy (Гловер — 46 упражнений + тест), games-people-play (Берн), love-languages (Чепмен), eq-2-0 (Бредберри/Гривз — тест), seven-principles (Готтман — тест), razgovorny-gipnoz (Бакиров), redecision-therapy (Гулдинги), 100-notes (Осипов — тест), radical-forgiveness (Типпинг), mind-power (Кехо)
- **Маркетинг / навык** (3 книги): pishi-sokraschay (Ильяхов — пирамида коммуникации, калибровка), heroes-and-outlaws (Марк/Пирсон — 12 архетипов, тест), borba-za-vnimanie (Белоусов — методика «Работа с контекстами», калибровка)

Новые книги добавляются через INSERT в БД (programs + program_modes), UI data-driven.
Продакшен: nice-guy-ai.vercel.app

## Мини-проекты — критическое правило

Папка `mini/` содержит **изолированные мини-проекты** (отдельные тесты, лендинги, демки — например, тест-воронка Волынского). Они НЕ часть основного приложения и подчиняются собственным правилам в `mini/<slug>/CLAUDE.md`.

Папка исключена из дефолтного сканирования через `.claudeignore`.

### Когда пользователь работает над ОСНОВНЫМ проектом (дефолт)
- НЕ читай файлы из `mini/**` (ни Read, ни Glob, ни Grep — без явного пути от пользователя).
- НЕ предлагай изменений в `mini/**`.
- Если поиск возвращает результат из `mini/` — игнорируй его в ответе.
- Если пользователь спрашивает «покажи как работает funnel», «найди тест Карины» — это явный сигнал переключить контекст (см. ниже).

### Когда пользователь явно просит работать над мини X
Триггеры: «работа над `mini/funnel`», «правки в funnel», «измени мини X», или сам пользователь открыл/назвал файл из `mini/<slug>/`.

В этом режиме:
- Читай **только** `mini/<slug>/CLAUDE.md` + `mini/<slug>/README.md` как контекст.
- ИГНОРИРУЙ этот корневой CLAUDE.md — правила основного проекта (чат, режимы, портрет, скиллы `niceguy-design`/`chat-rules`/`book-to-modes`) к мини НЕ применяются.
- Не читай файлы из других `mini/<other-slug>/` — это другой проект.
- Шимы в `app/<slug>/...` — это автогенерированные re-export'ы для роутинга Next.js (1-2 строки на файл). Не редактируй вручную; вся логика живёт в `mini/<slug>/`.

### Текущий список мини-проектов
- `mini/funnel/` (маршрут `/funnel`) — тест-воронка Волынского _(пока живёт по старой схеме в `app/funnel/` + `lib/funnel/` + `components/funnel/`; миграция в `mini/` — отдельной задачей)_
...
- **Supabase** — PostgreSQL + Auth (Magic Link, Telegram Login Widget, Яндекс OAuth, Google OAuth) + RLS
...
### Архитектурный инвариант чатов

AI-сообщения рендерятся через два компонента из `components/chat/ChatMessage.tsx`:
- `<AIBubble>` — пузырь с markdown (внутри flex-row контейнера `.nc-msg`/`.msg`)
- `<QuickReplyBar>` — блок кнопок-«ёлочек», рендерится как **SIBLING** после `.nc-msg`/`.msg`, НЕ внутри

Парсер «ёлочек» — единый: `lib/chat/parse-quick-replies.ts`. Вызывающий код парсит один раз, передаёт `cleanText` → AIBubble, `replies` → QuickReplyBar.

- ❌ Нельзя рендерить AI-ответ через `<ReactMarkdown>` напрямую в чат-компоненте
- ❌ Нельзя копировать логику парсинга «ёлочек» в компонент
- ❌ Нельзя создавать новый чат-экран без `<AIBubble>` + `<QuickReplyBar>`
- ❌ **Нельзя** ставить `<QuickReplyBar>` внутри `.nc-msg`/`.msg` (уедет в узкую колонку сбоку) — только как sibling через `<Fragment>`

Причина: дублирование рендера приводит к рецидивам («ёлочки» работают в одном компоненте, ломаются в другом). Неправильное размещение кнопок (внутри flex-row контейнера) даёт узкую колонку вместо полноширинного блока. Подробности: `docs/chat-audit-eq-2-0.md`.
...
- **Сквозные тексты бренда** (название платформы, ярлыки секций лендинга, подписи режимов): источник истины — `docs/brand-glossary.md`. При любом переименовании сквозной фразы:
  1. Обнови словарь `docs/brand-glossary.md` (старая фраза в «Запрещено», новая в «Каноническая лексика»).
  2. Обнови шаблоны в скиллсе `book-to-modes` (`SKILL.md`, `references/PLATFORM_MAP.md`) и в `docs/runbooks/add-new-book.md`.
  3. Обнови `lib/platform-landing.ts` и все `scripts/seed-*.sql` (чтобы пересоздание БД давало правильный результат).
