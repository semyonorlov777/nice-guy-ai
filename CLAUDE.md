...
Все AI-сообщения рендерятся через **единый компонент** `components/chat/ChatMessage.tsx`. Парсер «ёлочек» — `lib/chat/parse-quick-replies.ts`.
...
- ❌ Нельзя создавать новый чат-экран без переиспользования `<ChatMessage>`
- Если нужна кастомизация — расширь ChatMessage, не создавай обход

Причина: 2026-04-23 обнаружено что дублирование логики рендера приводит к рецидивам — «ёлочки» работают в одном компоненте, ломаются в другом. Подробности в `docs/chat-audit-eq-2-0.md`.
