...
export const metadata: Metadata = {
  title: "Демо E: название книги в заголовке",
  robots: { index: false, follow: false },
};

const SLUG = "nice-guy";

// Главное отличие от A: заголовок Hero — это название книги, а не обещание.
// Eyebrow сверху говорит «ИНТЕРАКТИВНЫЙ ТРЕНАЖЁР ПО БЕСТСЕЛЛЕРУ» — логично,
// что главная строка ниже — название этого бестселлера. Привязка к понятному
// становится мгновенной. Обещание-посыл уходит в приветствие чата.
const HERO_EYEBROW = "✦ Интерактивный тренажёр по бестселлеру";
const HERO_TITLE = "Хватит быть<br>славным парнем";

// В карточке книги название не повторяем — оно сверху как H1.
...
// Приветствие — обещание-посыл (раньше было в заголовке) переезжает сюда.
// Это даёт контекст «что я дам», а не голое обещание на первом экране.
const HERO_WELCOME =
  "Привет. Я тренажёр по методу Роберта Гловера. Помогу перестать жить ради чужого одобрения и начать жить по своим правилам.\n\nРасскажи одну ситуацию: где ты последний раз сказал «да», когда хотел сказать «нет»?";
...
// Маленькая подпись над «ёлочками» — как в стандартных чатах.
const QUICK_REPLY_LABEL = "Можно начать с одного из этих:";
...
export default async function DemoLandingE() {
  ...
  return (
    <div className="landing-v3 demo-landing">
      <div className="demo-bar">
        ...
        <span className="demo-bar__tag">Вариант E · название книги в заголовке</span>
      ...
      <HeroChatSectionE
        ...
        quickReplyLabel={QUICK_REPLY_LABEL}
      ...
      <section className="demo-why-block">
        <div className="demo-why-block__inner">
          ...
          <ul className="demo-why-block__list">
            <li>Заголовок Hero — это <strong>название книги</strong> «Хватит быть славным парнем», а не обещание. Eyebrow сверху уже задал контекст («тренажёр по бестселлеру»), и следующая крупная строка — название этого бестселлера. Привязка к понятному считывается за полсекунды.</li>
            <li>В блоке книги (рядом с обложкой) название не повторяется — освобождает воздух, чище композиция.</li>
            <li>Обещание-посыл переезжает в приветствие чата: «Помогу перестать жить ради чужого одобрения». Это становится контекстом «что я дам», а не маркетинговой водой на первом экране.</li>
            <li>«Ёлочки» — в проектном стиле (золотые кнопки), с маленькой подписью сверху «Можно начать с одного из этих:» — как в стандартных чатах.</li>
          </ul>
