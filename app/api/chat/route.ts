...
export async function POST(request: Request) {
  ...
  try {
    ...
    // 6.5. QR-reminder ДОЛЖЕН быть последним в промпте (recency effect).
    // Иначе topicContext / portrait / test scores перекрывают QR-правила,
    // и Gemini перестаёт ставить «ёлочки» (особенно в темах где context в самом конце).
    systemPrompt += `\n\n---\nНАПОМИНАНИЕ О ФОРМАТЕ ОТВЕТА (ВАЖНО, ПОВТОРИ СЕБЕ ПЕРЕД ОТВЕТОМ):\n1) 2 абзаца, 60-80 слов, ОДИН вопрос за ход.\n2) В КОНЦЕ каждого ответа — 3-4 варианта ответа пользователя в «ёлочках», каждый на отдельной строке, последний «Мне сложно сформулировать» или тематический exit.\n3) НИКОГДА не используй <угловые скобки> вместо «ёлочек» — парсер их не видит, кнопки не появятся.\n4) НИКОГДА не склеивай «ёлочки» через пробел в одну строку.\n5) Если исключение — ходы 13+ по scaffolding, разрешено 0-1 reply.`;
